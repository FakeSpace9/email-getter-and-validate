    ...
    async with async_playwright() as p:
        ...
        for num in numbers:
            ...
            try:
                ...
                # Focus the main typing box (using .last to ensure we don't grab hidden overlays)
                print("⌨️ Focusing the main chat box...")
                main_text_box = page.locator('div#main div[contenteditable="true"][role="textbox"]').last
                ...
                await asyncio.sleep(1.0) # Increased slightly to ensure PowerShell finishes putting it on clipboard
                ...
                # FIX: Use .last instead of .first because modals are added to the end of the DOM.
                # .first captures the hidden send button behind the modal, which causes the timeout.
                send_button_icon = page.locator('span[data-icon="send"], div[aria-label="Send"]').last

                await asyncio.sleep(1.5) # Brief pause to ensure WhatsApp grabs the text as a caption

                # --- STEP 3: SEND (Image + First Text) ---
                print("🚀 Sending image...")
                # NATIVE FIX: WhatsApp auto-focuses the caption box. Hitting Enter is the most reliable way to send.
                await page.keyboard.press("Enter")

                # Click Fallback: If it's still on screen after 2 seconds (meaning Enter failed), try clicking.
                await asyncio.sleep(2)
                if await send_button_icon.is_visible():
                    print("🔄 Enter didn't dismiss modal, trying click fallback...")
                    try:
                        # Target the parent div of the send icon and click it
                        await page.locator('div[role="button"]:has(span[data-icon="send"])').last.click(timeout=3000)
                    except:
                        pass
